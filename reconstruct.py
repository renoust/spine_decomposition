...
class reconstruct():
	...
	def createGraphs(self):
		...
		for g in keys:
			...
			sg.setName("Level "+str(g))
			self.k2g[g] = sg
			for t in obj[str(g)]:
				if len(obj[str(g)][t]["nodes"]) == 0:
					continue
				ssg = sg.addSubGraph()
				ssg.setName(str(t))
				if str(t) == "spine":
					...
					self.k2rest[g] = ssg
				id2n = {}
				viewLabel = ssg.getRoot().getStringProperty("viewLabel")
				viewLayout = ssg.getLayoutProperty("viewLayout")
				idP = ssg.getDoubleProperty("original id")
				neighP = ssg.getStringProperty("neighbors")
				levelP = ssg.getDoubleProperty("level")
				
				for n in obj[str(g)][t]["nodes"]:
					node = ssg.addNode()
					viewLabel[node] = str(n["label"])
					viewLayout[node] = tlp.Coord(n["pos"][0], n["pos"][1], 0)
					idN = int(n["id"]) 
					idP[node] = idN
					id2n[idN] = node
					
					neighList = [str(k) for k in n["neighbors"]]
					#print n["label"], ";".join(neighList)
					neighP[node] = ";".join(neighList)
					
					if idN not in self.idToNList:
						self.idToNList[idN] = []
						
					if str(t) == "spine":
						levelP[node] = int(g)
						if idN not in self.idToFirst:
							self.idToFirst[idN] = g
						self.idToLast[idN] = g							
						
					else:
						levelP[node] = int(g) - 1

					self.idToNList[n["id"]].append(node)
				
				if str(t) == "spine":
					self.levelToID2Node[int(g)] = id2n
				else:
					self.levelToRID2Node[int(g)-1] = id2n
								
				for e in obj[str(g)][t]["edges"]:
					s = id2n[e["s"]]
					t = id2n[e["t"]]
					ssg.addEdge(s, t)
		
		vLayout = sg.getLayoutProperty("viewLayout")
		sg.computeLayoutProperty("Sugiyama (OGDF)", vLayout)
		
	def setMain(self):
		sg = self.graph.addSubGraph()
		sg.setName("Main")
		vLM = sg.getLayoutProperty("viewLayout")
		vCM = sg.getColorProperty("viewColor")
		vLCM = sg.getColorProperty("viewLabelColor")
		vLCMBcp = sg.getColorProperty("viewLabelColorBcp")
		vCMBcp = sg.getColorProperty("viewColorBcp")
		viewLabelBcp = sg.getStringProperty("viewLabelBcp")
		
		vShM = sg.getIntegerProperty("viewShape")
		vSM = sg.getSizeProperty("viewSize")
		viewLabel = sg.getStringProperty("viewLabel")

		idPM = sg.getDoubleProperty("original id")
		neighPM = sg.getStringProperty("neighbors")
		levelPM = sg.getDoubleProperty("level")

		colorKey = "RdYlBu"#"PuBuGn"#"Set3"
		colorSet = colorbrewer[colorKey][7]#len(self.k2g)]
		
		margin = 50
		deltaX = 0
		deltaY = 0
		
		for k in sorted(self.k2g.keys()):
			width = 0
			height = 0
			
			if k in self.k2spine:
				spine = self.k2spine[k]
				idP = spine.getDoubleProperty("original id")
				vL = spine.getLayoutProperty("viewLayout")
				neighP = spine.getStringProperty("neighbors")
				levelP = spine.getDoubleProperty("level")
				...
				maxY = pos[1]
				col = colorSet[int(k)]
				currentColor = tlp.Color(col[0],col[1],col[2])


				for n in spine.getNodes():
					pos = vL[n]
					if pos[0] < minX:
						minX = pos[0]
					if pos[0] > maxX:
				...
				for n in spine.getNodes():
					...
					pos = vL[n]
					pos[0] += (deltaX + margin) - minX
					pos[1] -= (deltaY + margin) + maxY
					vLM[n] = pos
					
					idN = idP[n]
					firstSpine = self.idToFirst[idN]
					col = colorSet[int(firstSpine)]
					vCM[n] = tlp.Color(col[0], col[1], col[2])
					vCMBcp[n] = tlp.Color(col[0], col[1], col[2])
					vLCMBcp[n] = vLCM[n]
					viewLabelBcp[n] = viewLabel[n]
					vShM[n] = 1
					
					occ = len(self.idToNList[idN])
					vSM[n] = tlp.Size(occ, occ, occ)
				...
				for e in spine.getEdges():
					sg.addEdge(e)
					vCM[e] = currentColor
					vCMBcp[e] = currentColor
				
			
			if k in self.k2rest:
				rest = self.k2rest[k]
				idP = rest.getDoubleProperty("original id")
				vL = rest.getLayoutProperty("viewLayout")
				neighP = rest.getStringProperty("neighbors")
				levelP = rest.getDoubleProperty("level")
				...
				nListID = [idP[n] for n in rest.getNodes()]
				for n in rest.getNodes():
					sg.addNode(n)
					idPM[n] = idP[n]
					levelPM[n] = levelP[n]
				...
				for e in rest.getEdges():
					sg.addEdge(e)
					vCM[e] = tlp.Color(0,0,0)
					vCMBcp[e] = tlp.Color(0,0,0,)
				...
				for n in rest.getNodes():
					...
					hashnid = self.levelToID2Node[int(levelP[n])]
					#print self.levelToRID2Node.keys()
					#print self.levelToRID2Node[int(levelP[n])]
					dd = self.levelToRID2Node[int(levelP[n])]
					hashnid.update(dd)

					ccList = []
					prevLen = 0
					if rest.deg(n) > 0:
						newList = [x for x in rest.getInOutNodes(n) if x not in ccList and x.id != n.id]
						for nnn in newList:
							ccList.append(nnn)
						while newList:
							resList = []
							for nnn in newList:
								for x in rest.getInOutNodes(nnn): 
									if x not in ccList and x.id != n.id:
										ccList.append(x)
										resList.append(x)
							newList = resList
					print ccList
					...
					tList = {int(idP[x]):x for x in ccList}
					...
					nList = neighP[n]
					if len(nList) == 0 and len(tList) == 0:
					...
					nList = nList.split(';')	
					for ng in nList:
						if int(ng) in hashnid:
							n2 = sg.addNode()
							viewLabel[n2] = viewLabel[hashnid[int(ng)]]
							e = sg.addEdge(n, n2)
							vCM[e] = tlp.Color(0,0,0)
							metanodelist.append(n2)
							if len(tList) > 0:
								newToN[int(ng)] = n2
						else:
							print "possible issue"
					
					for ng in tList.keys():
						ng = int(ng)
						if ng in newToN:
							nn = newToN[ng]
						else:
							#print "id ",idN
							#print "direct ng ",tList
							#print "used ng ",nList
							#print "current direct ng ",ng
							#print "map ",newToN.keys()
							#print "whole level ",sorted(hashnid.keys())
							nn = sg.addNode()
							corrNode = hashnid[int(ng)]
							lbl = viewLabel[corrNode]
							viewLabel[nn] = lbl
							metanodelist.append(nn)
							newToN[ng] = nn

						ntList = neighP[hashnid[int(ng)]].split(";")
						for nng in ntList:
							nng = int(nng)
							if nng not in newToN:
								nn2 = sg.addNode()
								viewLabel[nn2] = viewLabel[hashnid[int(nng)]]
								metanodelist.append(nn2)
								newToN[nng] = nn2
							else:
								nn2 = newToN[nng]
								
							ee = sg.existEdge(nn, nn2, False)
							if not ee.isValid():
								ee = sg.addEdge(nn, nn2)
								vCM[ee] = tlp.Color(0,0,0)
						
					mn = sg.createMetaNode(set(metanodelist), True, True)
					viewLabel[mn] = viewLabel[n]
					vCM[mn] = tlp.Color(col[0], col[1], col[2])
					vShM[mn] = 1
					vSM[mn] = tlp.Size(occ, occ, occ)
					vLM[mn] = pos
					vCMBcp[mn] = tlp.Color(col[0], col[1], col[2])
					vLCMBcp[mn] = vLCM[mn]
					viewLabelBcp[mn] = viewLabel[mn]
					...
					ssg = sg.getNodeMetaInfo(mn)
					vL2 = ssg.getLayoutProperty("viewLayout")
					ssg.computeLayoutProperty("GEM (Frick)", vL2)
...
def main(graph): 
	x = reconstruct("/work/abello_spine_twitte2r_temp.json", graph)
